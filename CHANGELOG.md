...
- Cleanup releases transfer and target descriptors before it starts an LED timer. The manager maps the existing `ERROR_TYPE` values `device_unknown`, `lock_timeout`, `no_space`, `rsync`, and `verify_failed` to the common LED helpers. The broad PID lock and `remove`-path `pkill` limitations remain scheduled for #8/PR10 and #16.

### Verified
- This PR's recorded local verification is limited to five suites: `test-target-manager.sh` (21 scenarios, 209 assertions), `test-status.sh` (9 scenarios, 47 assertions), `test-backup-core.sh` (10 scenarios, 114 assertions), `test-config.sh` (20 scenarios, 92 assertions), and `test-led.sh` (8 scenarios, 64 assertions).
