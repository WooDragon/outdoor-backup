...
目标存储的配置命令和运维流程以 [README.md 的 Configuration 章节](README.md#configuration) 为权威入口。守卫拓扑、FD 锚定、LuCI 字段边界和已知限制见 [docs/component-implementation.md](docs/component-implementation.md)。LuCI 的目标存储选择只从已验证的已挂载候选中生成 UCI 配置；它不替代 `fstab` 管理，也不替代运行时目标守卫。修改该选择流程前，应先读取 [docs/component-implementation.md](docs/component-implementation.md)、[docs/WEBUI_USER_GUIDE.md](docs/WEBUI_USER_GUIDE.md) 和 [docs/WEBUI_DEVELOPER_GUIDE.md](docs/WEBUI_DEVELOPER_GUIDE.md)。LuCI 目标发现只在固定 helper 子进程边界释放继承的 FD 9；父进程描述符与共用目标守卫保持不变。目标选择器采用原生 select，字段名仍遵循 CBI 契约。来源身份记录固定在已锚定 `TARGET_BACKUP_ROOT/.card-identities/<SD_UUID>.json`，并只表示来源文件系统 UUID 的首次观察绑定，不表示物理卡身份。修改加载或守卫逻辑前，应读取 [config.sh](files/opt/outdoor-backup/scripts/config.sh)、[card-config.sh](files/opt/outdoor-backup/scripts/card-config.sh)、[target.sh](files/opt/outdoor-backup/scripts/target.sh)、[target-device.sh](files/opt/outdoor-backup/scripts/target-device.sh)、[card-identity.sh](files/opt/outdoor-backup/scripts/card-identity.sh)、[source-identity.sh](files/opt/outdoor-backup/scripts/source-identity.sh)、[backup-manager.sh](files/opt/outdoor-backup/scripts/backup-manager.sh) 及对应测试。
