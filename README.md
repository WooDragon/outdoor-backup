...
After the initial read-only mount, the manager separately rereads the source filesystem UUID for the existing card-binding record. That read remains the identity value for the binding; it does not replace the pre-lock snapshot. A failed snapshot capture or recheck stops the run as `device_unknown` without a later source operation or completed status.
