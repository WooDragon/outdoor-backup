...
`test-cleanup.sh` 只可在一次性隔离测试目录中以显式 `--force` 清理破坏性 fixture。无 `--force` 的拒绝门必须保留。测试不得在真实备份目录中运行。该测试合同不改变生产清理脚本。
...
`outdoor-backup/Makefile` 是核心包的 canonical feed recipe。OpenWrt feed 只索引子目录中的配方，因此根目录不再提供核心包 `Makefile`。`outdoor-backup/files` 保持指向 `../files` 的相对链接。根 `files/` 仍是所有核心包运行时内容的唯一来源。LuCI 包继续使用 `luci-app-outdoor-backup/Makefile`。
